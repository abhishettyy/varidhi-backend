...
def build_fishing_recommendation_plan(
    ...
    steps: List[PlanStep] = [
        ...
        PlanStep(
            ...
            depends_on=[],
            required=False,
            parameters={"location": location, "time_range": time_range},
    ...
    for var in vars_to_plan:
        ...
        data_steps.append(
            PlanStep(
                ...
                depends_on=[],
                required=True,
                parameters={"variable": var_upper, "location": location, "time_range": time_range},
...
async def planner_node(state: MarineState) -> Dict[str, Any]:
    ...
    # Preserve legacy expected keywords in plan list for older tests
    if intent == MarineIntent.FISHING_RECOMMENDATION.value:
